...
LOCAL_MODEL_URL="${JUNIE_LOCAL_UPDATE_FILES_BASE_URL-https://raw.githubusercontent.com/jetbrains-junie/junie/main/local}/install.sh"
...
install_local_model() {
  ...
  if ! curl -fsSL "$LOCAL_MODEL_URL" | bash; then
    ...
    # and it is the part most likely to be refused -- it needs macOS 26 on an
    # Apple M5 or newer with 40 GB of RAM. Whatever went wrong here, Junie itself
    # is installed, so say so plainly instead of leaving the run looking failed.
    echo ""
    log_error "The local model setup did not complete."
    log "Junie itself is installed and ready to use -- run: junie"
    log "To try the local model again: curl -fsSL $LOCAL_MODEL_URL | bash"
    exit 1
  fi
}
